...
class Student(models.Model):
    ...
    def student_marks_report_data(self, evals: tuple, notes, total_coef=0, for_stats=False, pv=False):
        ...
        for note in notes:
            ...
            data.setdefault('notes', {})[note.eval] = note.note if note.note in [0, -1] else None
            ...
            if len(evals) == 6:
                ...
                for m in moys:
                    if m is not None:
                        t += m
                        n += 1
                moy = formated_float(t / n) if n else 0
    ...
    def student_reportcard_data(self, evals: tuple, total_coef: int, lv2: str, lv3: str, notes):
        ...
        for note in notes:
            ...
            data = matieres_data[matiere.id]
            data['matiere'] = matiere
            data.setdefault('notes', {})[note.eval] = None if note.note in [0, -1] else note.note
            ...
            if len(evals) == 6:
                ...
                for i in range(0, 6, 2):
                    ...
                    if a is not None and b is not None:
                        ...
                        m = None
                    data[f'moy{i // 2 + 1}'] = m if m is not None else "/"
                    moys.append(m)
