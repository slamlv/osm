...
class ListView(AdminRequired, WithUsersSchoolSchema, BaseListView):
    pass
...
class BaseStaffMemberTimetable(View):
    ...
    def get(self, *args, **kwargs):
        ...
        context = {'title': title, 'time_table': timetable, 'days': ("Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi"),
                   'recap': staffmember_recap[0], 'total_heures': staffmember_recap[1], 'id': self.kwargs.get('id')}
    ...
    def post(self, *args, **kwargs):
        ...
        if empty_timetable:
            data['tranches_horaires'], data['school_data'] = staffmember.timetable(mp, school_id, empty=True)
            data['filename'] = "Emploi du temps individuel"
        else:
            data['time_table'], data['infos'], data['school_data'], data['recap_and_total'] = (
...
def resize_image(image_path, new_width=295, quality=85, return_height=False, id_card=False):
