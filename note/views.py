...
def classrooms_set(request):
    staff_member = request.user.staff_member.all()[0]
    if staff_member.rapporteur.all().exists():
        enseignements = Enseignements.objects.select_related('matiere__sujet', 'classroom').filter(
            rapporteur_id=staff_member.pk)
    select_form = SelectForm(request.GET, context={"request": request, 'trim': False, 'enseignements': enseignements})
...
class ExamRecord(FPDF):
    ...
    def infos(self):
        ...
        self.ln(3)
        self.cell(0, 10, f"**PROCÈS VERBAL {self.data['trimestre']}**", align='C', markdown=True)
        self.set_font_size(9)
        self.ln()
        self.cell(99, 5, f"**Classe : {self.data['label']}**", align='L', markdown=True)
        info = f"Effectif : {self.data['effectif']}, Filles : {self.data['filles']}, Garçons : " \
               f"{self.data['garcons']}, Redoublants : {self.data['redoublants']}"
    ...
    def head(self):
        ...
        self.image(logo, x=86, y=6, w=26, keep_aspect_ratio=True)
    ...
    def footer(self):
        ...
        self.cell(99, 10, f"PROCÈS VERBAL {self.data['trimestre'].title()} ({self.data['label']}) - Page "
...
class TMarksReport(FPDF):
    ...
    def infos(self):
        self.set_font_size(13)
        self.ln(3)
        self.cell(285, 10, f"**RELEVÉ DE NOTES {self.data['trimestre']}**", align='C', markdown=True)
        self.set_font_size(9)
        self.ln()
        self.cell(142.5, 5, f"**Classe : {self.data['label']}**", align='L', markdown=True)
        info = f"Effectif : {self.data['effectif']}, Filles : {self.data['filles']}, Garçons : " \
               f"{self.data['garcons']}, Redoublants : {self.data['redoublants']}"
        self.cell(142.5, 5, f"__{info}__", align='R', markdown=True)

    def head(self):
        school_data = self.data['school_data']
        hwidths = (117.5, 50, 117.5)
        table = Table(self, line_height=5, col_widths=hwidths, text_align="CENTER", first_row_as_headings=False,
                      borders_layout="NONE", markdown=True)
        logo = resize_image(school_data['logo'], new_width=308)
        self.image(logo, x=135.5, y=6, w=26, keep_aspect_ratio=True)
    ...
    def footer(self):
        self.set_y(-10)
        self.line(6, 202, 291, 202)
        self.set_font('inter', 'I', 8)
        self.set_x(10)
        self.cell(138.5, 10, f"Document généré par Oméga School Manager le {self.now}", align='L')
        self.cell(138.5, 10, f"RELEVÉ DE NOTES {self.data['trimestre'].title()} ({self.data['label']}) - Page "
                             f"{self.page_no()}/{{nb}}", align='R')
...
class ReportCard(FPDF):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(len(data['students_data'])):
            ...
            self.draw_student_reportcard(self, data, i, school_data, self.trimestre, annee, classroom_data,
                                         matieres_data, total_coef, min_max, moy_gen, taux, nb, nb_admis,
                                         chef=school_data['chef'])
            if self.nb_pages > 1 and i < len(data['students_data']) - 1:
    ...
    def footer(self):
        ...
        self.set_font('inter', 'I', 8)
        self.cell(100, 10, f"Document généré par OSM le {self.now}", align='L')
        if self.nb_pages > 1:
            page = (self.nb_pages, self.page_no() % self.nb_pages)[self.page_no() % self.nb_pages != 0]
            if page != 1:
                self.cell(90, 10, f"BULLETIN {self.trimestre.title()} ({self.nom}) - Page {page}/{self.nb_pages}",
                          align='R')
            else:
                self.cell(90, 10, f"BULLETIN {self.trimestre.title()} - Page {page}/{self.nb_pages}", align='R')
        else:
            self.cell(90, 10, f"BULLETIN {self.trimestre.title()}", align='R')
    ...
    def school_infos(self, pdf, school_data):
        ...
        table = Table(pdf, line_height=3.5, col_widths=widths, text_align="CENTER", first_row_as_headings=False,
                      borders_layout="NONE", markdown=True)
        row = table.row()
        row.cell("RÉPUBLIQUE DU CAMEROUN")
        row.cell("", rowspan=7)
        ...
        row = table.row()
        row.cell(f"{school_data['departement']}")
        row.cell(f"**Immatriculation : {school_data['immatriculation']}**\n__Tél : {school_data['contact']}__",
                 rowspan=3)
        ...
        row = table.row()
        row.cell(f"**{school_data['nom']}**")
        row.cell(f"**{school_data['name']}**")

        pdf.image(school_data['logo'], x=92, y=8, w=26, keep_aspect_ratio=True)
        table.render()
    ...
    def infos(self, pdf, trimestre, annee):
        ...
        pdf.cell(0, 2, f"Année scolaire : {annee}", align='C')
        pdf.ln()
    ...
    def student_infos(self, pdf, student_data, classroom_data):
        ...
        row = table.row()
        row.cell(
            img=resize_image(student_data['photo'] if student_data['photo'] else "static/image/student.jpg"),
    ...
    def discipline_height_and_table(self, pdf, discipline, total_notes, appr, total_coef, moyenne, rang, cote, moy_gen, taux, min_max,
                   nb, nb_admis, appreciation, chef):
        """
        Simule un tableau Table() temporairement pour connaître sa hauteur finale.
        La simulation n'est pas visible dans le PDF final.
        """
