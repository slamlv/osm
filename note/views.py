...
class LivretBEPC(LoggedAdminView):
    template_name = "students_import.html"
    title = "Remplissage du Livret BEPC"
    ...
    # Feuilles à ne jamais traiter (première et dernière du template)
    SHEETS_IGNOREES = {"COUVERTURE", "DERNIERE_PAGE"}

    EVALS_ANNUELLES = (1, 2, 3, 4, 5, 6)
    ...
    COL_MOY_T2 = 36
    COL_MOY_T3 = 37
    MAX_MATIERES = COL_NOTES_END - COL_NOTES_START + 1

    ROW_MATIERE_HEADERS = 4
    ROW_FIRST_ELEVE = 5
    ...
    def _detect_specialites(self, wb):
        specialites = []
        for sheet_name in wb.sheetnames:
            if sheet_name in self.SHEETS_IGNOREES:
                continue
            lv2 = self.SHEET_TO_LV2.get(sheet_name)
            if lv2:
                specialites.append((sheet_name, lv2))
        return specialites

    def _get_classrooms_for_lv2(self, lv2_value):
        from classroom.models import ClassRoom
        return (
            ClassRoom.objects
            .prefetch_related('students', 'matieres__sujet')
            .filter(classe__niveau='Troisième', lv2=lv2_value)
            .order_by('code')
        )

    def _get_matieres_labels(self, classroom, lv2_value):
        matieres = list(classroom.matieres.all().order_by_domain_and_coef(serie=""))
        labels = []
        for matiere in matieres:
            if matiere.sujet.label == "LVII":
                labels.append(lv2_value)
            else:
                labels.append(matiere.sujet.label)
        return labels

    def _build_notes_dataframe(self, classrooms, matieres_labels):
        nb_matieres = len(matieres_labels)
        rows = []
        ...
        for classroom in classrooms:
            ...
            for student_data in students_data:
                ...
                row = {'infos': student_data.get('student')}

                for i in range(nb_matieres):
                    val = self._to_float_or_none(mat_values[i].get('moy')) if i < len(mat_values) else None
                    row[f'mat_{i}'] = val

                # Totaux et moyennes globales renvoyés par student_reportcard_data
                row['total_notes'] = self._to_float_or_none(student_data.get('total_notes'))
                row['moyenne'] = self._to_float_or_none(student_data.get('moyenne'))
                row['moyenne1'] = self._to_float_or_none(student_data.get('moyenne1'))
                row['moyenne2'] = self._to_float_or_none(student_data.get('moyenne2'))
                row['moyenne3'] = self._to_float_or_none(student_data.get('moyenne3'))

                rows.append(row)

        return DataFrame(rows)

    def _write_sheet(self, ws, matieres_labels, df_notes):
        nb_matieres = len(matieres_labels)

        for i, label in enumerate(matieres_labels):
            ws.cell(row=self.ROW_MATIERE_HEADERS, column=self.COL_NOTES_START + i).value = label

        lookup = {}
        #print(df_notes.itertuples(index=False).__next__()._fields())
        for row_data in df_notes.itertuples(index=False):
            key = getattr(row_data, 'infos', None)
            if key:
                lookup[key.upper()] = row_data

        excel_row = self.ROW_FIRST_ELEVE
        while True:
            cell_b = ws.cell(row=excel_row, column=2).value
            if not cell_b:
                break

            key_excel = str(cell_b).strip().upper()
            row_data = lookup.get(key_excel)
            ...
            for i in range(nb_matieres):
                val = getattr(row_data, f'mat_{i}', None)

                ws.cell(
                    row=excel_row,
                    column=self.COL_NOTES_START + i
                ).value = None if (val != val) else val

            # Colonnes fixes de droite : Total Général + 4 moyennes
            ws.cell(row=excel_row, column=self.COL_TOTAL).value = self._to_float_or_none(row_data.total_notes)
            ws.cell(row=excel_row, column=self.COL_MOY_ANN).value = self._to_float_or_none(row_data.moyenne)
            ws.cell(row=excel_row, column=self.COL_MOY_T1).value = self._to_float_or_none(row_data.moyenne1)
            ws.cell(row=excel_row, column=self.COL_MOY_T2).value = self._to_float_or_none(row_data.moyenne2)
            ws.cell(row=excel_row, column=self.COL_MOY_T3).value = self._to_float_or_none(row_data.moyenne3)

            excel_row += 1

    def fill_bepc_livret(self, file):
        wb = load_workbook(file)

        specialites = self._detect_specialites(wb)

        if not specialites:
            return f"Aucune feuille de spécialité reconnue dans le fichier (attendu : {list(self.SHEET_TO_LV2.keys())})."

        for sheet_name, lv2_value in specialites:
            ws = wb[sheet_name]

            classrooms = list(self._get_classrooms_for_lv2(lv2_value))

            if not classrooms:
                continue

            matieres_labels = self._get_matieres_labels(classrooms[0], lv2_value) if not self.MATIERES else self.MATIERES

            nb_matieres = len(matieres_labels)
            if nb_matieres > self.MAX_MATIERES:
                raise ValueError(
                    f"Feuille {sheet_name} : {nb_matieres} matières détectées, "
                    f"mais seulement {self.MAX_MATIERES} colonnes disponibles (C→AF)."
                )

            df_notes = self._build_notes_dataframe(classrooms, matieres_labels)

            self._write_sheet(ws, matieres_labels, df_notes)
    ...
    def get(self, *args, **kwargs):
        return render(self.request, self.template_name, context={'title': self.title, 'livret': True})

    def post(self, *args, **kwargs):
        file = self.request.FILES.get('file')
        if file:
            result =  self.fill_bepc_livret(file)
            if isinstance(result, str):
                message(self.request, result, msg_type='error')
            else:
                response = HttpResponse(result.read(),
                                        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
                response['Content-Disposition'] = f'attachment; filename={file.name[:-5]}_REMPLI.xlsx'
                return response
        return render(self.request, self.template_name, context={'title': self.title, 'livret': True})
...
class ExamRecord(FPDF):
    ...
    def students_results(self):
        ...
        if self.data['annual']:
            col_widths = (9, 20, 70, 10, 10, 13, 13, 13, 13, 13, 35, 35, 21)
        ...
        self.cell(w / 4, 5, f"Filles : **{self.data['ppf']}% ({self.data['nbfe']} / {self.data['filles']})**" if self.data['ppf'] else "Filles : **/**", markdown=True)
        self.cell(w / 4, 5, f"Garçons : **{self.data['ppg']}% ({self.data['nbge']} / {self.data['garcons']})**"  if self.data['ppg'] else "Garçons : **/**", markdown=True)
        self.ln()
        self.cell(w / 2, 5, f"Taux de réussite : **{self.data['taux']}% ({self.data['nbr']} / {self.data['nbe']})**" if self.data['taux'] else "Taux de réussite : **/**", markdown=True)
        self.cell(w / 4, 5, f"Filles : **{self.data['pcf']}% ({self.data['nbfr']} / {self.data['nbfe']})**" if self.data['pcf'] else "Filles : **/**", markdown=True)
        self.cell(w / 4, 5, f"Garçons : **{self.data['pcg']}% ({self.data['nbgr']} / {self.data['nbge']})**" if self.data['pcg'] else "Garçons : **/**", markdown=True)
        self.ln()
...
class ReportCard(FPDF):
    ...
    def annual_student_notes(self, pdf, matieres_data, total_coef, total_notes, student_notes, moyenne, groupes,
        ...
        for groupe in groupes:
            ...
            while matiere_data['groupe'] == groupe:
                tr = table.row()
                label = truncate_str(pdf, matiere_data['label'], 31)
                tr.cell(f"**{label}**", align="LEFT")
                enseignant = truncate_str(self, matiere_data['enseignant'], 31)
    ...
    def discipline(self, pdf, discipline, total_notes, appr, total_coef, moyenne, rang, cote, moy_gen, taux,
        ...
        tr.cell(f"Nombre de moyennes >= {self.seuil}", rowspan=2)
