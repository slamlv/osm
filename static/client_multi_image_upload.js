// client_multi_image_upload.js (version robuste checkbox clear)
(() => {
  ...
  async function resizeAndConvert(file) {
    ...
    try { ctx.drawImage(imgBitmap, 0, 0, targetWidth, targetHeight); }
      ...
      await new Promise((res, rej) => {
        const i = new Image();
        i.onload = () => { ctx.drawImage(i, 0, 0, targetWidth, targetHeight); URL.revokeObjectURL(tmpUrl); res(); };
        i.onerror = (e) => { URL.revokeObjectURL(tmpUrl); rej(e); };
        i.src = tmpUrl;
      });
  ...
  function setupForm(form) {
    ...
    fileInputs.forEach(input => {
      ...
      input.addEventListener('change', async (e) => {
        ...
        } catch { compressedMap.delete(input); preview.style.display='none'; if(!info){ info=document.createElement('div'); info.className='text-white'; info.style='font-size:12px;margin-top:4px;color:inherit'; input.insertAdjacentElement('afterend',info);} info.textContent='Erreur lors du traitement côté client'; }
      });
    });
    ...
    form.addEventListener('submit', async (ev) => {
      ...
      // --- AJOUT FICHIERS COMPRESSES, sauf si clear est cochée ---
      fileInputs.forEach(input=>{
        const arr = compressedMap.get(input);
        if (!arr || arr.length===0) return;
        const clearChecked = form.querySelector(`input[name="${input.name}-clear"]`)?.checked;
        if (clearChecked) return; // clear prime sur le fichier
      ...
      xhr.upload.onprogress=(ev)=>{if(ev.lengthComputable){const pct=Math.round(ev.loaded/ev.total*100); prog.bar.style.width=pct+'%'; prog.text.textContent=`${pct}%`;} else prog.textContent='Envoi...';};
