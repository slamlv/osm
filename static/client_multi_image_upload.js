// client_multi_image_upload.js - compatible {{ form.photo }}
(() => {
  ...
  async function resizeAndConvert(file) {
    let imgBitmap;
    try { imgBitmap = window.createImageBitmap ? await createImageBitmap(file) : await new Promise((res, rej) => { const i=new Image(); i.onload=()=>res(i); i.onerror=rej; i.src=URL.createObjectURL(file); }); }
    catch { imgBitmap = await new Promise((res, rej)=>{ const i=new Image(); i.onload=()=>res(i); i.onerror=rej; i.src=URL.createObjectURL(file); }); }
    ...
    catch { const tmp=URL.createObjectURL(file); await new Promise((res,rej)=>{ const i=new Image(); i.onload=()=>{ctx.drawImage(i,0,0,targetWidth,targetHeight); URL.revokeObjectURL(tmp); res(); }; i.onerror=e=>{ URL.revokeObjectURL(tmp); rej(e); }; i.src=tmp; }); }
    finally { try { if(imgBitmap && imgBitmap.close) imgBitmap.close(); } catch{} }

    return await new Promise(resolve=>{ canvas.toBlob(blob=>{ if(!blob && OUTPUT_TYPE==='image/webp') canvas.toBlob(b2=>resolve(b2),'image/jpeg',QUALITY); else resolve(blob); },OUTPUT_TYPE,QUALITY); });
  }

  function blobToFile(blob,filename){ try{return new File([blob],filename,{type:blob.type});} catch{ blob.name=filename; return blob; } }

  async function navigateAndRender(url){ try{ const resp=await fetch(url,{credentials:'same-origin',method:'GET'}); if(!resp.ok){ window.location.assign(url); return;} const ct=resp.headers.get('Content-Type')||''; if(!ct.includes('text/html')){ window.location.assign(url); return;} const html=await resp.text(); history.replaceState(null,'',url); document.open(); document.write(html); document.close(); } catch{ window.location.assign(url); } }
  ...
  function setupForm(form){
    ...
    fileInputs.forEach(input=>{
      ...
      input.addEventListener('change',async e=>{
        ...
        }catch{ compressedMap.delete(input); preview.style.display='none'; if(!info){ info=document.createElement('div'); info.className='text-white'; info.style='font-size:12px;margin-top:4px;color:inherit'; input.insertAdjacentElement('afterend',info);} info.textContent='Erreur traitement image côté client'; }
      });
    });
    ...
    form.addEventListener('submit',async ev=>{
      ...
      fileInputs.forEach(input=>{
        ...
        const clearChecked = clearCheckbox?.checked;

        if(clearChecked){
          // clear prime: on supprime les fichiers compressés
          compressedMap.delete(input);
        } else {
          // si pas clear, on remplace par le fichier compressé
      ...
      xhr.upload.onprogress=(ev)=>{if(ev.lengthComputable){ const pct=Math.round(ev.loaded/ev.total*100); prog.bar.style.width=pct+'%'; prog.textContent=`${pct}%`; } else prog.textContent='Envoi...';};

      xhr.onload=async ()=>{
        controls.forEach(c=>c.disabled=false);
        if(xhr.responseURL){ await navigateAndRender(xhr.responseURL); return; }
        if(xhr.status>=200 && xhr.status<300){ prog.bar.style.width='100%'; prog.textContent='Téléversement terminé'; }
